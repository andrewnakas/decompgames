...
The candidate build is still private. Muted browser tests now establish menu input, entry to the gameplay loop, one committed player rail, a second station appearing with game time, and train dispatch. The independently drafted scenario also starts with three connecting rails between the first two stations. The train's movement was observed; successful delivery is under a separate private CI test and is not yet established. Release still requires a completable objective, save/reload/import, wider visual review, and the final provenance review. It remains outside the five-release count.
