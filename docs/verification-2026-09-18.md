...
## September 22: Digger release candidate

Rebuilt the pinned source without `--emrun`, exported the filesystem methods required by the shared player, added an IDBFS mount, and packaged immutable revision `e85cab1164f0-b7e87a0852` with its exact corresponding-source archive. A muted 40-second second-level run showed enemies, one lost life, and a successful restart. A separate storage test wrote, flushed, reloaded, read, deleted, and re-flushed a sentinel through the mounted filesystem.

The local Decomp Games integration downloaded and launched the packaged engine, completed the deterministic first level, rendered the second level, kept Sound off, exported a correctly namespaced empty backup before any score or preference file existed, and stopped cleanly. The immutable runtime and exact-source archive were uploaded to R2 and the catalog was deployed to Cloudflare Pages. The same muted launch, first-level completion, second-level rendering, backup export, and Stop game flow passed at `play.decompgames.com`. No SDL sound-device initialization appeared. High-score entry and a complete eight-level playthrough remain unverified and are not claimed. Open Digger now counts as strict addition 1/5.

## September 22: Open Paths production release

Published Open Paths package `bad56a4e174e-026301f2dd`, built from OpenSupaplex revision `bad56a4e174e628643995284ea55d4c49af3137c` with Emscripten 4.0.10. The runtime uses the null audio backend and preloads only 14 allowlisted, independently generated CC0 files. The exact corresponding-source archive SHA-256 is `df925352659c51ee6c0ac3f202b37b23b4bbbe8a4b0a56c62fe619951e7071a0`.

Muted local and production headless Microsoft Edge tests passed title/menu input, player creation, rover movement, the deterministic two-step first-level completion, completed-level progress after a fresh engine instance, a two-file backup export, namespace deletion, backup import, and restored progress. The production WASM matched SHA-256 `8bde797a8c381d8436d4cdada2703734bbc4c691a325c21290d68fac2ef4eaa5`; it served as `application/wasm` with byte ranges. The source archive matched its manifest and served as `application/gzip`. No sound device initialized. A full six-level playthrough remains unverified. Open Paths counts as strict addition 2/5.
