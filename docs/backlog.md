...
reSL's documented ShortLine decompilation now has independent CC0 external assets, ten generated replacement visual/mask tables, a replacement SDL cursor, and a candidate independent board, entrance set, train roster, and rail redraw bounds. The complete replacement set compiled privately in GitHub Actions run 35815903124 with a null audio backend. Muted browser smoke runs 35824375580 and 35824790463 confirmed runtime initialization and a 640×480 canvas without page errors or off-origin requests, but subsequent state reads stalled after input. The engine's melody flag was disabled for the second run. The menu's idle timeout still attempted to load an omitted original demo save and may explain the timing; the next private recipe removes that path for isolation. Input, gameplay, motion-data provenance, a completable loop, and persistence remain open; this build is not counted toward additions 3–5. See `docs/resl-replacement-integration.md` for pinned revision, artifact hashes, and release blockers.
