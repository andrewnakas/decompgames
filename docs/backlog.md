...
reSL's documented ShortLine decompilation now has independent CC0 external assets, ten generated replacement visual/mask tables, a replacement SDL cursor, and a candidate independent board, entrance set, train roster, and rail redraw bounds. The complete replacement set compiled privately in GitHub Actions run 35815903124 with a null audio backend. Muted browser smoke runs 35824375580, 35824790463, and 35825286995 confirmed runtime initialization and a 640×480 canvas without page errors or off-origin requests, but subsequent state reads stalled. Passive run 35825608587 proved the stall also occurs without input: reads succeeded at 3, 8, and 13 seconds, then the 18-second read hung. The next build removes the main-menu timeout decrement entirely. Independently authored entrance colors and a matching runtime palette are now generated but not yet compiled or reviewed in-game. Gameplay, motion-data provenance, a completable loop, and persistence remain open; this build is not counted toward additions 3–5. See `docs/resl-replacement-integration.md` for pinned revision, artifact hashes, and release blockers.
