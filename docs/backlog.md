...
- [Tube (Bullfrog) reconstruction](https://github.com/rep-stosw/tube-game-dos) is an actionable engine lead rather than an exclusion: the maintainer documents Hex-Rays/IDA reconstruction, GPL-2.0, SDL2, and a working Emscripten build. The repository tree also contains many original `DATA/` and `SOUND/` files plus compiled data segments. Audit its loaders and design independent replacement levels/textures/music before a feasibility build; never copy its packaged binary or original data to the site. The linked [tube64](https://github.com/rep-stosw/tube64) is the same game and must not be counted again.
