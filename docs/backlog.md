...
Latest private regression: [run 35866841564](https://github.com/andrewnakas/decompgames/actions/runs/35866841564) passed delivery, save/reload/Archive, and backup deletion/import after the sparse-forest and colored-tree changes. The 640×480 gameplay frame was reviewed; other screens, smaller labels, a complete campaign objective, the remaining functional-data audit, corresponding-source packaging, and a test through the actual player UI remain open. No runtime binary or site entry was promoted.
...
- [nitecrwl Atari ST decompilation](https://github.com/SegHaxx/nitecrwl) describes decompiled game code, but the inspected 10-commit repository has no README or license file and only a small C++/header/Makefile tree. The original game identity, rights, assets, and playable port are not established. Do not host or count it without an explicit code grant and a complete independent game package.
- [Chase H.Q. Spectrum reconstruction](https://github.com/dpt/ChaseHQ) is translated function by function from a documented Z80 disassembly and already runs in WebAssembly, but the inspected repository does not declare an open-source license for its rebuilt C code. Its current game presentation also follows the original. Hold hosting and counting until an explicit code grant and independent full replacement content are available. The author's [Great Escape reconstruction](https://github.com/dpt/The-Great-Escape) likewise had no license grant in the inspected top-level project.
- [Golden Balloon](https://github.com/akratch/goldenballoon) documents a Mario Kart 64 decompilation-based browser port, but its own README requires a user-supplied ROM and says no game textures, audio, models, or levels are included. No independently authored complete replacement pack was found; it is a local-file prospect, not an instant-play addition.
- [SkyRoads SDL](https://github.com/pedrocatalao/skyroads-sdl) describes a rewritten C/SDL engine and an original-data fetch. Its README says the original code carries no open-source license and only the port's own scripts and docs are MIT. The complete game data is Bluemoon's freeware, not independently authored replacements. It cannot meet this milestone's engine-and-assets gate.
- [Open Fodder](https://github.com/OpenFodder/openfodder) is GPL-3.0 and already has a browser demo, but upstream calls it an engine reimplementation, not a documented decompilation. Its [installation guide](https://github.com/OpenFodder/openfodder/blob/master/INSTALL.md) requires files from a magazine demo or retail release. No complete independent replacement data pack was found in this pass. Keep it outside additions 3–5 unless both provenance and new assets change.
- [Tube (Bullfrog) reconstruction](https://github.com/rep-stosw/tube-game-dos) is an actionable engine lead rather than an exclusion: the maintainer documents Hex-Rays/IDA reconstruction, GPL-2.0, SDL2, and a working Emscripten build. The pinned loader and packaged-data inventory are recorded in [`tube-replacement-feasibility.md`](tube-replacement-feasibility.md). In particular, startup reads a 624,880-byte `DSEG3.bin` memory image as well as level, texture, and sound files. Map that segment and every data loader before designing independent replacements; never copy the upstream `index.data` or original files to the site. The linked [tube64](https://github.com/rep-stosw/tube64) is the same game and must not be counted again.
- [SkyRoads disassembly-based Web port](https://github.com/haroldo-ok/skyroads-32x/tree/df219e03b854153291b32224593013b60d861d40) is a distinct engine lead from the excluded, unlicensed `skyroads-sdl` rewrite above. Its LICENSE text grants MIT rights to the new source, and the README traces the engine to the original executable's disassembly. It requires Bluemoon's original `*.LZS`, `*.DAT`, `*.SND`, and demo files. The documented `make_track.py` splices a custom track into original `ROADS.LZS`, so it does **not** supply full independent replacement content. The recovered [format specification](https://github.com/haroldo-ok/skyroads-32x/blob/df219e03b854153291b32224593013b60d861d40/docs/FORMATS.md) covers graphics and road containers but leaves some object, speedometer, and demo details open. Audit loaders and design independent art, interface, and levels before any hosting; deduplicate its upstream `ammaarreshi/skyroads-port` as the same game.
...
- [reVC Web](https://github.com/origami-ltd/wasm-revc) already compiles the Vice City decompilation for browsers, but its own README says the underlying reVC game logic has no license grant for distribution beyond the upstream authors' educational/modding request. The MIT grant covers only the browser layer. It also streams a visitor's installed commercial game files. Do not treat the existing browser port as an open-source, replacement-asset release.
- [pokeemerald WASM](https://github.com/tripplyons/pokeemerald-3d) is an in-progress browser port of the matching Pokémon Emerald decompilation. The port deliberately retains upstream original data and asset build pipelines; the related [rp2350 fork](https://github.com/mattdeeds/pokeemerald-rp2350) explicitly says its MIT license covers only new port code, not the upstream game source. No complete independent replacement Pokémon campaign or open-source grant for the decompiled game was established. Keep it outside the strict instant-play milestone.
