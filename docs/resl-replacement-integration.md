...
The traced private run [35828810324](https://github.com/andrewnakas/decompgames/actions/runs/35828810324) reached runtime initialization and reproduced the 13–18-second stall, but emitted no main-menu frame traces. Source review explains why that trace was too late: `showLoadingScreen()` normally cycles six caption phases with 220 retraces each plus transitions, about 24 seconds at 60 FPS. The stalled renderer may still be in that animation. The next private recipe shortens the independently authored title card to about one second, then retests idle responsiveness and traces the menu. This is an experiment, not a verified fix.
