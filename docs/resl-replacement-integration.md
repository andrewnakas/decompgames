...
The manual CI workflow now also includes a **private browser smoke test** with the null audio driver and Chromium's mute flag. It checks runtime initialization, canvas size, page errors, and off-origin requests before and after sending Enter. It uploads no images. Even if it passes, those observations establish only browser startup and possible input delivery; they cannot certify menu progression, gameplay, persistence, or redistribution readiness. The first attempt timed out during Chromium screenshot capture; a second attempt stalled during pixel inspection and was cancelled after three minutes. The harness now avoids both operations, logs each stage, and has a 45-second process limit.
