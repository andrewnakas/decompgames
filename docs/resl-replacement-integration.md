...
The next asset boundary is the original-derived scenario data under `src/game/resources/`. `allowed_cursor_rail_types` restricts the build grid, and `entrance_rails` defines a 46-rail starting layout; these look like level design rather than reusable engine rules. `train_specification` holds campaign train compositions and values. A replacement campaign should independently define all three, with its own title and level goals, before distribution. `movement_paths`, connection rules, biases, and rail metadata appear to support engine motion and geometry; their role needs a separate functional-data review. This classification is provisional, not a legal clearance.
