...
The corrected stage trace in [run 35829714926](https://github.com/andrewnakas/decompgames/actions/runs/35829714926) logged **after loading** and never reached **after create world**. Source review found a concrete flaw in the replacement scenario: all 23 sites in each entrance bank have the same horizontal coordinate, while upstream `generateEntrances()` demands a minimum 10-index gap between entrances in the same bank. For 14 of the 23 possible first choices in a bank, no two later choices can satisfy that rule, leaving its `while (!suits)` loop unbounded. The next candidate records six alternating, distinct, independently selected stations in the scenario manifest and patches the private build to use those choices. This is a plausible root-cause correction; it still needs a browser rerun before claiming the stall is fixed.
