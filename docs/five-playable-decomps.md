...
| Digger Remastered | https://github.com/sobomax/digger and https://www.digger.org/faq.html document remaster/source rights; its upstream WASM target builds at the pinned revision | Eight CC0 layouts plus independent graphics, font, title, and icon now compile in a no-audio WASM variant. Remaining static-data audit and a complete gameplay/readability pass still block release; see `docs/digger-replacement-integration.md`. |
