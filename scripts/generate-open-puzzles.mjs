...
for(let n=1;n<=6;n++) {
  ...
  tiles[61]=3;
  const exit=13*width+19;tiles[exit]=7;
  ...
  let steps=0;for(let p=exit;p!==61;p=previous.get(p))steps++;
  ...
  records.push({file,name,bytes:data.length,sha256:createHash('sha256').update(data).digest('hex'),shortestPathSteps:steps,engineTested:false});
}
await writeFile(resolve(out,'manifest.json'),JSON.stringify({license:'CC0-1.0',source:'scripts/generate-open-puzzles.mjs',originalAssetsUsed:false,scope:'Six navigation prototypes; not a full campaign or complete replacement asset pack.',levels:records},null,2)+'\n');
