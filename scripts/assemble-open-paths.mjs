...
await writeFile(resolve(output,'manifest.json'),JSON.stringify({id:'open-paths-experiment',license:'CC0-1.0',engineSourceRevision:'bad56a4e174e628643995284ea55d4c49af3137c',requiredEngineOption:'--replacement-pack',campaignLevels:6,releaseReady:false,blockers:['Incomplete animation coverage','No integrated gameplay or save verification','Missing audio policy integration'],files:staged.map(f=>({name:f.name,source:f.source,bytes:f.bytes.length,sha256:hash(f.bytes)}))},null,2)+'\n');
console.log(`Assembled ${staged.length} checked replacement files in ${resources}; experimental only.`);
