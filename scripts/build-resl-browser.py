...
if os.environ.get("OPEN_JUNCTION_TRACE") == "1":
    ...
    loop_text = loop_text.replace(
        ...
        + '    return -1;\n'
        + '}\n'
        + 'extern "C" EMSCRIPTEN_KEEPALIVE int oj_trace_fourth_switch_state() {\n'
        + '    const Rail* branch = &g_rails[1][4][4];\n'
        + '    for (int i = 0; i < g_nSwitches; ++i) {\n'
        + '        const Switch& sw = g_switches[i];\n'
        + '        if (sw.entry.rail == branch || sw.disabledPath.rail == branch)\n'
        + '            return ((sw.entry.rail == branch) ? 1 : 0) << 24\n'
        ...
        + '    return -1;\n'
        + '}\n'
        + 'extern "C" EMSCRIPTEN_KEEPALIVE int oj_trace_fifth_switch_state() {\n'
        + '    const Rail* branch = &g_rails[6][3][2];\n'
        + '    for (int i = 0; i < g_nSwitches; ++i) {\n'
        + '        const Switch& sw = g_switches[i];\n'
        + '        if (sw.entry.rail == branch || sw.disabledPath.rail == branch)\n'
        + '            return ((sw.entry.rail == branch) ? 1 : 0) << 24\n'
        ...
        + '    return -1;\n'
        + '}\n'
        + 'extern "C" EMSCRIPTEN_KEEPALIVE int oj_trace_sixth_switch_state() {\n'
        + '    const Rail* branch = &g_rails[3][6][4];\n'
        + '    for (int i = 0; i < g_nSwitches; ++i) {\n'
        + '        const Switch& sw = g_switches[i];\n'
        + '        if (sw.entry.rail == branch || sw.disabledPath.rail == branch)\n'
        + '            return ((sw.entry.rail == branch) ? 1 : 0) << 24\n'
        ...
        + '}\n'
        + 'extern "C" EMSCRIPTEN_KEEPALIVE void oj_trace_jump_to_1920() {\n'
        + '    g_headers[static_cast<int>(HeaderFieldId::Year)].value = 1920;\n'
        + '    g_gameTime = 1900;\n'
        + '    std::fprintf(stderr, "OJ year-1920 branch prepared\\n");\n'
        + '}\n'
        + 'extern "C" EMSCRIPTEN_KEEPALIVE void oj_trace_jump_to_1960() {\n'
        + '    g_headers[static_cast<int>(HeaderFieldId::Year)].value = 1960;\n'
        + '    g_gameTime = 1900;\n'
        + '    std::fprintf(stderr, "OJ year-1960 branch prepared\\n");\n'
        + '}\n'
    ...
    trains.write_text(train_text.replace(
        ...
        '    return slot;\n'
        '}\n'
        'extern "C" EMSCRIPTEN_KEEPALIVE int oj_trace_spawn_fifth_service() {\n'
        '    if (g_entranceCount < 5 || !noTrainsExist() || !entranceIsFree(3)) return -1;\n'
        '    Train* train = spawnTrain(3);\n'
        '    if (!train) return -2;\n'
        '    for (int i = 0; i < train->carriageCnt; ++i)\n'
        '        train->carriages[i].dstEntranceIdx = 4;\n'
        '    const int slot = static_cast<int>(train - g_trains.data());\n'
        '    std::fprintf(stderr, "OJ trace forced fifth service slot %d\\n", slot);\n'
        '    std::fprintf(stderr, "OJ train spawned from 3 to 4 at year %d slot %d\\n",\n'
        '        train->year, slot);\n'
        '    return slot;\n'
        '}\n'
        'extern "C" EMSCRIPTEN_KEEPALIVE int oj_trace_spawn_sixth_service() {\n'
        '    if (g_entranceCount < 6 || !noTrainsExist() || !entranceIsFree(4)) return -1;\n'
        '    Train* train = spawnTrain(4);\n'
        '    if (!train) return -2;\n'
        '    for (int i = 0; i < train->carriageCnt; ++i)\n'
        '        train->carriages[i].dstEntranceIdx = 5;\n'
        '    const int slot = static_cast<int>(train - g_trains.data());\n'
        '    std::fprintf(stderr, "OJ trace forced sixth service slot %d\\n", slot);\n'
        '    std::fprintf(stderr, "OJ train spawned from 4 to 5 at year %d slot %d\\n",\n'
        '        train->year, slot);\n'
        '    return slot;\n'
        '}\n\n' + next_train_function,
        1,
