...
await writeFile(resolve(out,'manifest.json'),JSON.stringify({license:'CC0-1.0',originalAssetsUsed:false,engineCoordinateMetadataUsed:Boolean(animationSource),engineTested:false,multiTileTransitions:'descriptor-driven rover',campaignScope:'Open Paths uses navigation and exit tiles only; unreachable collectible, hazard and enemy interactions are intentionally outside this asset set.',regions,limitations:['Player animation coordinates use one original geometric rover design','The six-level Open Paths campaign intentionally excludes collectibles, hazards and enemies','A release still requires in-engine campaign verification'],file:{name:'MOVING.DAT',bytes:bytes.length,sha256:createHash('sha256').update(bytes).digest('hex')}},null,2)+'\n');
console.log('Generated the Open Paths moving atlas; browser campaign verification remains required.');
